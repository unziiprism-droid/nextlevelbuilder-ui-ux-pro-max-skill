...
发布说明和 `CHANGELOG.md` 根据约定式提交信息生成。在发布准备期间，版本号会在 `skill.json`、`.claude-plugin/plugin.json`、`.claude-plugin/marketplace.json`、`cli/package.json` 和 `cli/package-lock.json` 之间同步。

使用以下提交类型以获得正确的版本号升级：

- `fix:` -> 补丁版本发布
- `feat:` -> 次版本发布
- `feat!:` 或 `BREAKING CHANGE:` -> 主版本发布
